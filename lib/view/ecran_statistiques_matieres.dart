...
class EcranStatistiquesMatieres extends StatelessWidget { //dans cette view on a opté pour Provider au lieu de Setstate car on a plusieurs widgets dans l’écran qui doivent rebuild ensemble à partir d’une même source de données → impossible avec un seul setState.
  ...
  Widget build(BuildContext context) {
    final vm = Provider.of<StatsMatiereViewModel>(context);
    return GradientScaffold(
      body: SafeArea(
        child: ValueListenableBuilder<Map<String, Map<String, dynamic>>>(
          valueListenable: vm.paiementsParMatiere,
          builder: (_, map, __) {
  ...
  /* ---------- Widget carte par matière ---------- */
  Widget _matiereCard(String matiereTitre, int abonnes) {
    return Card(
      elevation: 3,
